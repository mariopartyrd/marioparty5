#ifndef _INST_EFFECT_H
#define _INST_EFFECT_H

#include "game/hu3d.h"

typedef struct InstEffect_s {
    int parManId[3];
} INST_EFFECT;

static INST_EFFECT *ParticleDotCreate(ANIMDATA *animP, s16 layer, s16 cameraBit)
{
    ...
    INST_EFFECT *effectP;
    effectP = HuMemDirectMallocNum(HEAP_HEAP, sizeof(INST_EFFECT), HU_MEMNUM_OVL);
...
static void ParticleDotPosSet(INST_EFFECT *effectP, HuVecF *pos)
{
...
static void ParticleDotKill(INST_EFFECT *effectP)
{
...
static INST_EFFECT *ParticleGlowCreate(ANIMDATA *animP, s16 layer, s16 cameraBit)
{
...
static void ParticleGlowPosSet(INST_EFFECT *effectP, HuVecF *pos)
{
    Hu3DParManPosSet(effectP->parManId[0], pos->x, pos->y, pos->z);
    Hu3DParManTimeLimitSet(effectP->parManId[0], 2);
    Hu3DParManAttrReset(effectP->parManId[0], HU3D_PARMAN_ATTR_TIMEUP);
...
static void ParticleGlowKill(INST_EFFECT *effectP)
{
    Hu3DParManKill(effectP->parManId[0]);
}
