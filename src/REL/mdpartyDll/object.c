...
void MdCameraCreate(MDCAMERA_WORK *cameraWork)
{
    MDCAMERA_TYPE_INFO type[MDCAMERA_MAX] = {
        ...
        { HU3D_CAM2, MDCAMERA_FOCUS_TARGET },
    };
...
void MdMessWinScaleSet(MDMESS_WORK *mess, int winNo, float posX, float posY)
{
    MDMESS_WIN *win = &mess->win[winNo];
    if(win->winId != HUWIN_NONE) {
        HuWinScaleSet(win->winId, posX, posY);
    }
}
...
void MdMessWinCenterPosSet(MDMESS_WORK *mess, int winNo, float posX, float posY)
{
    MDMESS_WIN *win = &mess->win[winNo];
    if(win->winId != HUWIN_NONE) {
        HuWinCenterPosSet(win->winId, posX, posY);
    }
}
...
void MdMessWinMesSet(MDMESS_WORK *mess, int winNo, BOOL choice, ...)
{
    ...
    if(win->winId != HUWIN_NONE) {
        ...
        for(i=0; i<HUWIN_INSERTMES_MAX; i++) {
            ...
            if(mesTbl[i] == MDMESS_WIN_LIST_END) {
                break;
            }
            num++;
        }
